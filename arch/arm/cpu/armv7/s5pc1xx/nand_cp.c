 ...
 *
 * 2013-6-9 添加了nandll_read_n_byte，改善了读的速度。256k只需要3秒钟就可以
 * copy完
 * 2013-6-25 进行的优化，去掉无关的注释
 * 2013-6-27 在获取id前的延时进行了延长，因为以前没有考虑到主频升到1GHz的情况。会有
 *           id获取失败的情况
 *
 */
 ...
 */
static int puthex(volatile unsigned char a)
{
 ...
 * 判断一个数组前n项是否为递增
 */
static int isInc(uint* a, int len)
{
	if( len > 0 )
	{
		if( len == 1 )
 ...
 * 修复Main区的反转位
 */
static int fixEcc(uchar* buf, int num, int flag)
{
	uint subst[16];
	uchar pattern[16];
	int i = 0;
 ...
 * 读512Byte并进行ECC校验
 */
static uchar nand_read_512_ecc(uchar *buf, unsigned int addr, uchar num)
{
	uint ret = 0;
	uchar ecctemp[26];
	uchar ECCErrorNo = 0;
	...
	{
		putcc('E');putcc('R');putcc('R');putcc('O');putcc('R');putcc('!');
		putcc('\n');putcc('\r');
		putcc('>');putcc('1');putcc('6');putcc('b');putcc('i');putcc('t');
		putcc('\n');putcc('\r');
 ...
 * 这里要加上static才能启动完整
 */
static int copy_uboot_to_ram_nand (void)
{
	int large_block = 0;
	int i;
	vu_char id;
...
#if 0
static void cleanup_before_uboot(void)
{
	//disable_interrupts();
	//icache_disable();
	//dcache_disable();
...
void board_init_f_nand(unsigned long bootflag)
{
	...
	putcc('\n');putcc('\r');
	putcc('B');putcc('L');putcc('1');putcc(' ');putcc('V');putcc('e');putcc('r');putcc(':');
	putcc('1');putcc('3');putcc('0');putcc('6');putcc('2');putcc('5');
	putcc('\n');putcc('\r');
	putcc('S');putcc('t');putcc('a');putcc('r');putcc('t');putcc(' ');putcc('c');putcc('p');putcc(' ');
	putcc('\n');putcc('\r');
...
#if 0
	for(buf=CONFIG_SYS_TEXT_BASE; buf<CONFIG_SYS_TEXT_BASE+COPY_BL2_SIZE; buf++)
		puthex(*buf);
#endif
	putcc('R');putcc('u');putcc('n');putcc('n');putcc('i');putcc('n');putcc('g');putcc('!');putcc(' ');
	putcc('\n');putcc('\r');
